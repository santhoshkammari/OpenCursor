...
<function>{"description": "Replace specified string in a non-Python file (like .md, .txt, .sh files). Use for updating specific content in text-based files or fixing errors in code.", "name": "file_str_replace", "parameters": {"properties": {"explanation": {"description": "One sentence explanation as to why this tool is being used, and how it contributes to the goal.", "type": "string"}, "file": {"description": "Path of the file to perform replacement on", "type": "string"}, "new_str": {"description": "New string to replace with", "type": "string"}, "old_str": {"description": "Original string to be replaced", "type": "string"}, "sudo": {"description": "Whether to use sudo privileges", "type": "boolean"}}, "required": ["file", "old_str", "new_str"], "type": "object"}}</function>

<note>
PRIORITIZE USING THE FILE_STR_REPLACE TOOL OVER THE EDIT_FILE TOOL FOR TEXT-BASED FILES (.md, .txt, .sh, etc.).
Don't use the EDIT_FILE TOOL for text-based files.
USE edit_file for python files only.
</note>
